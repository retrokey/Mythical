...
export const LoginView: FC<{  }> = props => {
    ...
    return (<div className="bg-gradient bg-opacity-75 w-screen h-screen">
        ...
        <div className="absolute w-[415px] h-auto rounded-[10px] bg-white bg-opacity-60 dark:bg-black dark:bg-opacity-60 p-3 top-1/4 left-1/3 flex flex-col items-center justify-between">
            ...
            <div className="relative w-full h-60 flex flex-col items-center justify-between">
                ...
                <button onKeyDown={ event => keyDown(event) } onClick={ event => submit() } className="relative w-full h-12 rounded-[8px] text-black dark:text-white bg-yellow">Enter</button>
                <button onClick={ event => openRegistration() } className="relative w-full h-12 rounded-[8px] text-black dark:text-white bg-gray">Registration</button>
            </div>
        </div>
    </div>);
}