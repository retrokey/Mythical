...
export const StaffView: FC<{  }> = props => {
    ...
    const founder = useMemo(() => {
        ...
        if (getStaff().fou.length == 0) {
            items.push(<p key="no-fou" className="absolute text-center text-opacity-80 dark:text-opacity-80 dark:text-white text-black w-full h-16px leading-16px font-inter font-semibold font-13px top-20px">We doesn't have founders!</p>);
        } else {
            for (let staff of getStaff().fou) {
                ...
                <div key={ staff.username } className="flex flex-row items-center bg-white dark:bg-black relative w-327px h-162px">
                    ...
                    <div className="absolute text-black dark:text-white text-opacity-80 dark:text-opacity-80 w-64px h-16px leading-16px font-inter font-semibold font-12px top-120px left-140px">{ staff.role }</div>
